...
describe Sinatra do
  ...
  it "should take empty junit, produce valid json" do
    post "/junit", ""
    expect(last_response).to be_ok
    # FIXME: this probably is not valid json
    expect(last_response.body).to eq('["test_run", {"project"=>nil, ' \
                                     '"duration"=>0.0, ' \
                                     '"configuration"=>[], ' \
                                     '"start_time"=>nil, ' \
                                     '"execution_id"=>nil, ' \
  ...
  it "should take good junit, produce good json" do
    ...
    post "/junit", File.read("spec/fixtures/beaker_junit_03.xml")
    expect(last_response).to be_ok
    expect(last_response.body)
      .to match(/\"start_time\"=>2016-08-01 23:09:46 UTC/)
